...
export default function AppMore() {
  ...
  return (
    <main className="min-h-screen bg-[#f3f6f8] pb-[calc(3rem+env(safe-area-inset-bottom))] text-[#101d2b]">
      ...
      <div className="px-6 pb-8 pt-8 text-center text-sm font-semibold text-black/35">
        <p>AthletiGolf app preview</p>
        <button type="button" onClick={() => navigate("/terms")} className="mt-4 block w-full text-pulse underline">
          Terms of Use
        </button>
        <button type="button" onClick={() => navigate("/privacy")} className="mt-2 block w-full text-pulse underline">
          Privacy Policy
        </button>
      </div>
    </main>
...
function MoreSection({ title }: { title: string }) {
  return (
    <div className="border-y border-black/5 bg-[#e9edf1] px-5 py-4">
      <p className="text-lg font-black tracking-tight text-[#142231]">{title}</p>
    </div>
  );
}
...
function MoreRow({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex min-h-[4.7rem] w-full items-center gap-5 border-b border-black/7 bg-white px-5 text-left transition active:bg-black/[0.03]"
    >
      <span className={`inline-flex h-10 w-10 shrink-0 items-center justify-center ${accent ? "text-pulse" : "text-[#142231]"}`}>
        <Icon className="h-7 w-7" />
      </span>
      <span className="min-w-0 flex-1">
        <span className={`block text-[1.12rem] font-medium leading-tight tracking-wide ${accent ? "text-pulse" : "text-[#142231]"}`}>
          {label}
        </span>
        {detail && <span className="mt-1 block truncate text-sm text-black/48">{detail}</span>}
      </span>
      {dot && <span className="mr-2 h-4 w-4 shrink-0 rounded-full bg-danger" />}
      <EndIcon className="h-6 w-6 shrink-0 text-[#142231]" />
    </button>
  );
}
