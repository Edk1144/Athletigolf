  ...
  return (
    <main className="min-h-screen bg-cream px-4 py-5 text-ink md:px-8 md:py-7">
      <PageHeader
        ...
        title="AthletiAI"
        description="A single review surface for golf trends, training signals, recovery context and next actions."
        tone="text-pulse"
      />
      ...
      <section className="mb-5 grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <Surface className="bg-dark text-white">
          <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-start">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-semibold tracking-tight">{performanceInsights[0]?.title || "Build the first signal"}</h2>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            <DarkMetric label="Rounds" value={rounds.length} />
            <DarkMetric label="Training" value={workouts.length} />
            <DarkMetric label="Data health" value={`${completeItems}/${dataHealth.length}`} />
          </div>
        </Surface>
        ...
        <Surface className="border-golf/20 bg-golf/5">
          ...
          <Button variant="golf" onClick={() => (window.location.href = recommendedPracticeHref)} className="mt-5">
            Start Practice Plan
          </Button>
        </Surface>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_1fr]">
        <Surface>
          ...
          <div className="grid gap-3">
            <CoachNote icon={Flag} title="Golf" text={coachNotes.golf} tone="golf" />
