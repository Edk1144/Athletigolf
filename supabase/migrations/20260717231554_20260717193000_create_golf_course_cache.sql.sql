/*
# Create golf course cache

1. Purpose
- Caches GolfCourseAPI course records selected by AthletiGolf users so repeated
  lookups avoid extra API calls and tee/hole data is available offline-ish.

2. New Tables
- `golf_courses`: cached course header (club, course, address, lat/lon, external_id, raw_data).
- `golf_course_tees`: cached tee sets with rating, slope, yardage, par totals. Unique on
  (course_id, gender, tee_name, number_of_holes). FK to golf_courses with cascade delete.
- `golf_course_holes`: cached per-hole par, yardage, meters, handicap. Unique on
  (tee_id, hole_number). FK to golf_course_tees with cascade delete.

3. Modified Tables
- `rounds`: adds golf_course_id, golf_course_external_id, golf_course_tee_id, tee_name,
  course_rating, slope_rating, total_yards, total_meters, par_total (all optional).
- `round_holes`: adds yardage, meters, handicap (all optional).

4. Indexes
- idx_golf_courses_external_id on golf_courses(external_id).
- idx_golf_courses_search GIN tsvector index on club/course/city/country text.
- idx_golf_course_tees_course_id on golf_course_tees(course_id).
- idx_golf_course_holes_tee_id on golf_course_holes(tee_id).
- idx_rounds_golf_course_id on rounds(golf_course_id).

5. Security
- RLS enabled on all three new tables.
- SELECT policies scoped TO authenticated with USING (true): golf course data is a shared
  reference catalog, intentionally readable by any signed-in user. No insert/update/delete
  policies — writes happen only via the service-role key from the golf-course-search edge
  function, which bypasses RLS.
- SELECT grants to authenticated on all three tables.
*/
